...
class PongGame(Turtle):
    ...
    def paddle_collision_detected(self, paddle: Paddle):
        ball_is_too_far_right = paddle.xcor() > 0 and self.ball.horizontal_direction > 0 and self.ball.xcor() > (
                    paddle.xcor() - 20)
        ball_is_too_far_left = paddle.xcor() < 0 and self.ball.horizontal_direction < 0 and self.ball.xcor() < (
                    paddle.xcor() + 20)

        return (ball_is_too_far_right or ball_is_too_far_left) and self.ball.distance(paddle) < 59

    def ball_is_off_edges(self, paddle: Paddle):
        is_off_right_edge = paddle.xcor() > 0 and self.ball.xcor() > (paddle.xcor() + 10)
        is_off_left_edge = paddle.xcor() < 0 and self.ball.xcor() < (paddle.xcor() - 10)

        return is_off_left_edge or is_off_right_edge

    def reset_towards_paddle(self, paddle):
        self.ball.goto(0, 0)
        self.ball.horizontal_direction = 1 if paddle.xcor() > 0 else -1
        self.ball.vertical_direction = random.choice([-1, 1])
        self.ball.speed_gain = 1
